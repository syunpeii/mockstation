...
2. 次に Test Case Search と Device Management を Server API に接続する
3. Home は集約表示なので、他画面の基盤ができた後に接続する
4. request history は HTTP 取得を先に実装し、WebSocket は後から追加する
5. ViewModel から直接 `HttpClient` を呼ばず、Repository / UseCase を通す
